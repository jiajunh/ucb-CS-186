...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.S);
        LeafNode rt = root.get(key);
        return new BPlusTreeIterator(rt, rt.scanGreaterEqual(key));
        // TODO(proj4_part3): B+ tree locking
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> out = root.put(key, rid);

        if(out.isPresent()){
            List<DataBox> keys = new ArrayList<DataBox>();
            keys.add(out.get().getFirst());

            List<Long> children = new ArrayList<Long>();
            children.add(root.getPage().getPageNum());
            children.add(out.get().getSecond());

            root = new InnerNode(metadata,bufferManager, keys, children, this.lockContext);
        }
        // TODO(proj4_part3): B+ tree locking
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        while(data.hasNext()) {
            Optional<Pair<DataBox, Long>> out = root.bulkLoad(data, fillFactor);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            this(root.getLeftmostLeaf(), Collections.emptyIterator());
        }

        public BPlusTreeIterator(LeafNode leaf, Iterator<RecordId> iter) {
            this.leaf = leaf;

            if(!iter.equals(Collections.emptyIterator())){
                this.iter = iter;
            } else{
                this.iter = leaf.scanAll();
            }
        }
