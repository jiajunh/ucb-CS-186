...
public class SortOperator {
    ...
    public Run sortRun(Run run) {
        ...
        ArrayList<Record> recVal = new ArrayList<Record>();
        for(Iterator<Record> iter = run.iterator(); iter.hasNext() ;){
            recVal.add(iter.next());
        }
        Collections.sort(recVal, comparator);
        Run sorted = createRun();
        sorted.addRecords(recVal);
        return sorted;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        ArrayList<Iterator<Record>> iters = new ArrayList<Iterator<Record>>();

        int i = 0;
        for(Run newVal : runs){
            Iterator<Record> newIter = newVal.iterator();
            if(newIter.hasNext()){
                pq.add(new Pair<>(newIter.next(), i));
                iters.add(newIter);
            }
            i++;
        }
        Run msRuns = createRun();


        while (!pq.isEmpty()){
            Pair<Record, Integer> pair = pq.remove();
            if (iters.get(pair.getSecond()).hasNext()){
                pq.add(new Pair<>(iters.get(pair.getSecond()).next(), pair.getSecond()));
            }
            msRuns.addRecord(pair.getFirst().getValues());
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int cons = numBuffers -1;
        for(int i = 0; i < runs.size(); i += cons){
            rlist.add(mergeSortedRuns(runs.subList(i, i + cons)));
        }
        return rlist;
    }
    ...
    public String sort() {
        // TODO(proj3_part1): implement
        Iterator<Page> iter = transaction.getPageIterator(tableName);
        ArrayList<Run> runs = new ArrayList<Run>();
        while(iter.hasNext()){
            BacktrackingIterator<Record> bit = transaction.getBlockIterator(tableName, iter, numBuffers);
            Run run = createRunFromIterator(bit);
            runs.add(sortRun(run));
        }
        while(runs.size() > 1){
            List<Run> mergeRun = mergePass((List) runs);
            runs = (ArrayList) mergeRun;
        }
