...
public class GraceHashJoin {
    ...
    private List<Record> buildAndProbe(HashPartition partition) {
        ...
        while (buildRecords.hasNext()) {
            Record leftRecord = buildRecords.next();
            DataBox leftJoinValue = leftRecord.getValues().get(buildColumnIndex);
        ...
        while (probeRecords.hasNext()) {
            Record rightRecord = probeRecords.next();
            DataBox rightJoinValue = rightRecord.getValues().get(probeColumnIndex);
    ...
    private HashPartition[] createPartitions() {
        ...
        HashPartition[] hp = new HashPartition[this.numBuffers-1];
        for (int i=0; i<hp.length; i++) {
            hp[i] = createPartition();
        }
        return hp;
    }
