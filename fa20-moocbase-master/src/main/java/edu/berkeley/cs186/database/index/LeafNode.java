...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() > 2 * metadata.getOrder()) {
            ...
            int len = (int) Math.floor(keys.size() / 2);
            List<DataBox> rkeys = keys.subList(len, keys.size());
            List<RecordId> rrids = rids.subList(len, rids.size());
            LeafNode rightNode = new LeafNode(metadata, bufferManager, rkeys, rrids, rightSibling, treeContext);

            keys = keys.subList(0, len);
            rids = rids.subList(0, len);
            rightSibling = Optional.of(rightNode.getPage().getPageNum());

            // rkeys[0] should be the smallest key in this LeafNode since sorted
            out = Optional.of(new Pair(rightNode.getKeys().get(0), rightNode.getPage().getPageNum()));
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while(data.hasNext() && !out.isPresent()){
            ...
            keys.add(pos, key);
            rids.add(pos , dataPT.getSecond());

            if (getKeys().size() > (int) (2 * metadata.getOrder() * fillFactor)) {

                int len = keys.size();
                List<DataBox> rkeys = keys.subList(len-1, len);
                List<RecordId> rrids = rids.subList(len-1, len);
                LeafNode rightNode = new LeafNode(metadata, bufferManager, rkeys, rrids, rightSibling, treeContext);

                keys = keys.subList(0, len-1);
                rids = rids.subList(0, len-1);
                rightSibling = Optional.of(rightNode.getPage().getPageNum());
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        long siblingpt = buf.getLong();
        Optional<Long> sibl =
                Optional.ofNullable(siblingpt == -1L ? null : siblingpt);
        int n = buf.getInt();


        List<DataBox> key = new ArrayList<>();
        List<RecordId> rid = new ArrayList<>();

        for (int i = 0; i < n; ++i) {
            key.add(DataBox.fromBytes(buf, metadata.getKeySchema()));
            long pgNum = buf.getLong();
            short ent = buf.getShort();
            rid.add(new RecordId(pgNum, ent));
        }

        return new LeafNode(metadata, bufferManager, page, key, rid, sibl, treeContext);
    }
