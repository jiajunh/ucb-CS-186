...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        pass1map.put(new HashSet<String>() {{ add(startTableName); }}, minCostSingleAccess(startTableName));
        for(int i = 0; i < joinTableNames.size(); i++) {
            String t = joinTableNames.get(i);
            QueryOperator newOp = minCostSingleAccess(t);
            pass1map.put(new HashSet<String>() {{ add(t); }}, newOp);
        }
        Map<Set<String>, QueryOperator> newMap = pass1map;
        ...
        // been joined.
        while (newMap.size() > 1){
            newMap = minCostJoins(newMap, pass1map);
        }
        ...
        finalOperator = minCostOperator(newMap);
        addGroupBy();
        addProjects();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        for (int idx : getEligibleIndexColumns(table)) {
            QueryOperator indOp = new IndexScanOperator(this.transaction, table, this.selectColumnNames.get(idx),
                    this.selectOperators.get(idx), this.selectDataBoxes.get(idx));
            if (indOp.estimateIOCost() < minOp.estimateIOCost()) {
                minOp = indOp;
                exidx = idx;
            }
        }
        ...
        return addEligibleSelections(minOp, exidx);
    }
    ...
    Map<Set<String>, QueryOperator> minCostJoins(Map<Set<String>, QueryOperator> prevMap,
         ...
         */
        for (Set<String> t : prevMap.keySet()) {
            for (int i=0; i<joinTableNames.size(); i++) {
                QueryOperator thisOp = prevMap.get(t);
                QueryOperator newOp;
                Set<String> key;
                ...
                String leftC = leftside[1];
                String leftT = leftside[0];
                String rightC = rightside[1];
                String rightT = rightside[0];

                if(t.contains(leftT) && !t.contains(rightT)){
                    Optional<Set<String>> r = pass1Map.keySet().stream()
                            .filter(l -> l.contains(rightT))
                            .findFirst();
                    key = r.get();
                    QueryOperator otherOp = pass1Map.get(key);
                    newOp = minCostJoinType(thisOp, otherOp, leftC, rightC);
                }
                else if(!t.contains(leftT) && t.contains(rightT)){
                    Optional<Set<String>> r = pass1Map.keySet().stream()
                            .filter(l -> l.contains(leftT))
                            .findFirst();
                    key = r.get();
                    QueryOperator otherOp = pass1Map.get(key);
                    newOp = minCostJoinType(otherOp, thisOp, leftC, rightC);
                } else{
                    continue;
                }

                retTables.addAll(t);
                retTables.addAll(key);
                int cost = newOp.estimateIOCost();
                int prevCost= Integer.MAX_VALUE;
                if (prevMap.containsKey(retTables)){
                    prevCost = prevMap.get(retTables).estimateIOCost();
                }

                int newCost = Integer.MAX_VALUE;
                if (map.containsKey(retTables)){
                    newCost = map.get(retTables).estimateIOCost();
                }

                if (cost < prevCost && cost < newCost){
                    map.put(retTables, newOp);
                }
