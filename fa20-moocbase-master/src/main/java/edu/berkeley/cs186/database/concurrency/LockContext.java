...
public class LockContext {
    ...
    public void acquire(TransactionContext transaction, LockType lockType)
        ...
        if (!this.getExplicitLockType(transaction).toString().equals("NL"))
            throw new DuplicateLockRequestException("Duplicate Lock");
        LockContext pa = this.parent;
        while (pa != null) {
            if (pa.getExplicitLockType(transaction).toString().equals("NL")) {
                break;
            }
            if (!LockType.canBeParentLock(pa.getExplicitLockType(transaction), lockType)) {
                throw new InvalidLockException("Invalid lock!");
            }
            pa = pa.parent;
        }
        if (lockType.toString().equals("NL"))
            return;
        this.lockman.acquire(transaction, getResourceName(), lockType);
        if (this.parent != null) {
            if (this.parent.numChildLocks.containsKey(transaction.getTransNum())) {
                Integer old = this.parent.numChildLocks.get(transaction.getTransNum());
                this.parent.numChildLocks.replace(transaction.getTransNum(), old + 1);
            } else {
    ...
    public void release(TransactionContext transaction)
        ...
        if (this.numChildLocks.containsKey(transaction.getTransNum()) &&
        ...
        if (this.parent != null) {
            if (this.parent.numChildLocks.containsKey(transaction.getTransNum())) {
                ...
                this.parent.numChildLocks.put(transaction.getTransNum(), 0);
            }
        }
        return;
    }
    ...
    public void promote(TransactionContext transaction, LockType newLockType)
        ...
        if (this.getExplicitLockType(transaction).equals(newLockType)) {
            throw new NoLockHeldException("Duplicate lock request!");
        }
        if (!(LockType.substitutable(newLockType, this.getExplicitLockType(transaction))
                || (newLockType.toString().equals("SIX") && (this.getExplicitLockType(transaction).toString().equals("IS")
                || this.getExplicitLockType(transaction).toString().equals("S") ||
                this.getExplicitLockType(transaction).toString().equals("IX"))))) {
            throw new InvalidLockException("Invalid lock!");
        }
        this.lockman.promote(transaction, getResourceName(), newLockType);
        if (newLockType.toString().equals("SIX")) {
            for (Lock lock: this.lockman.getLocks(transaction)) {
                if (lock.name.isDescendantOf(getResourceName()) &&
                        (lock.lockType.toString().equals("S") || lock.lockType.toString().equals("IS"))) {
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        LockType curLock = this.getExplicitLockType(transaction);
        List<ResourceName> releaseLocks = new ArrayList<>();
        releaseLocks.add(name);
        if (!this.numChildLocks.containsKey(transaction.getTransNum())) {
            if (curLock.toString().equals("IS") || curLock.toString().equals("SIX")) {
                lockman.acquireAndRelease(transaction, name, LockType.S, releaseLocks);
            } else if (curLock.toString().equals("IX")) {
        ...
        for (Lock lock: lockman.getLocks(transaction)) {
            if (lock.name.isDescendantOf(getResourceName()) &&
                    !lock.lockType.toString().equals("NL")) {
                releaseLocks.add(lock.name);
                if (lock.lockType.toString().equals("X") || lock.lockType.toString().equals("IX")) {
                    flag = true;
                }
            }
        }
    ...
    public LockType getExplicitLockType(TransactionContext transaction) {
        ...
        // TODO(proj4_part2): implement
        for (Lock lock: this.lockman.getLocks(name) ) {
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        if (this.parentContext() == null) {
            ...
            if (this.parentContext().getEffectiveLockType(transaction).toString().equals("S") ||
                    this.parentContext().getEffectiveLockType(transaction).toString().equals("X")) {
                return this.parentContext().getEffectiveLockType(transaction);
            } else {
                return this.getExplicitLockType(transaction);
            }
        }
