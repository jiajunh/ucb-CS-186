...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) throws BPlusTreeException {
        ...
        BPlusNode child = getChild(pos);
        Optional<Pair<DataBox, Long>> pushUp = child.put(key, rid);
        Optional<Pair<DataBox, Long>> out = Optional.empty();


        if (pushUp.isPresent()) {
            DataBox insert = pushUp.get().getFirst();

            int loc = keys.size();
            for (int i = 0; i < loc; i++) {
                if (insert.compareTo(keys.get(i)) < 0) {
                    loc = i;
                    break;
                }
            }
            ...
            keys.add(loc, insert);
            children.add(loc + 1, pushUp.get().getSecond());

            if (keys.size() > 2 * metadata.getOrder()) {
                int kPT = (int) Math.floor(keys.size() / 2);
                int cPT = (int) Math.floor(children.size() / 2);
                DataBox split = keys.get(kPT);

                List<DataBox> rkeys = keys.subList(kPT + 1, keys.size());
                List<Long> rchilds = children.subList(cPT, children.size());
                keys = keys.subList(0, kPT);
                children = children.subList(0, cPT);
                InnerNode newNode = new InnerNode(metadata, bufferManager, rkeys, rchilds, treeContext);

                out = Optional.of(new Pair<>(split, newNode.getPage().getPageNum()));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        Optional<Pair<DataBox, Long>> out = Optional.empty();
        while (data.hasNext() && !out.isPresent()) {

            Pair<DataBox, RecordId> dataPT = data.next();
            DataBox key = dataPT.getFirst();
            ...
            List<Pair<DataBox, RecordId>> item = new ArrayList<>();
            item.add(dataPT);
            Iterator<Pair<DataBox, RecordId>> iter = item.iterator();
            Optional<Pair<DataBox, Long>> pushUp = child.bulkLoad(iter, fillFactor);
            ...
            if (pushUp.isPresent()) {
                ...
                if (keys.size() > 2 * metadata.getOrder()) {
                    ...
                    out = Optional.of(new Pair<>(keys.get(kPT), newNode.getPage().getPageNum()));
